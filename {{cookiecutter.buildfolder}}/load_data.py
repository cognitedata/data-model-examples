...
def load(file, drop, example_folder):
    ...
    if file:
        ...
        for _, _, filenames in os.walk(f"./{example_folder}/data/"):
            for f in filenames:
                if ".csv" in f:
                    files.append(f)
    ...
    for f in files:
        with open(f"./{example_folder}/data/{f}", "rt") as file:
            ...
            try:
                ...
                print(f"Failed to upload {f} for example {example_folder}: {e.message}")
                exit(1)
        print(f"Successfully uploaded {f} to {raw_db}.")
    print(f"Successfully uploaded {len(files)} files to {raw_db}.")
